...
const earthjs = (options = {}) => {
    ...
    d3
        .selectAll(options.svgCanvasSelector)
        .attr('width', width)
        .attr('height', height);
                ...
                if (_.promeses.length > 0) {
                    ...
                    _.promeses.forEach(obj => {
                        obj.urls.forEach(url => {
                            let ext;
                            const match = url.match(/\.(geojson|json|csv)$/);
    ...
    __.refresh = function(filter) {
        ...
        if (filter) {
            const keys = filter
                ? _.onRefreshKeys.filter(d => filter.test(d))
                : _.onRefreshKeys;
